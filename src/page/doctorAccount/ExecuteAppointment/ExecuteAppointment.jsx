...
const ExecuteAppointment = () => {
  ...
  const handleUpdateAppointment = async () => {
    try {
      ...
      params.append("pet_id", data.pet.id);
      await http.put(`/appointments/${data.appointment_id}`, params);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data.vaccine_history.length > 0 ? (
                    data.vaccine_history.map((vaccine) => (
                      <tr key={vaccine.id ? vaccine.id : "new"}>
                        {" "}
                        {/* Added a key for each row */}
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                          {vaccine.id} {/* Fixed to display vaccine id */}
                        </td>
